...
;( function ( WIN )
	{
		...
		function MPTable( elem )
		{
			...
			GATOR( this.tbody )
				...
				.on( 'dblclick', '[data-song-path]', function ()
					{
						var alreadyPlaying = that.tbody.getElementsByClassName( 'mp-song-playing' )

						for ( var i = 0, l = alreadyPlaying.length; i < l; ++i )
						{
							alreadyPlaying[ i ].classList.remove( 'mp-song-playing' )
						}

						this.classList.add( 'mp-song-playing' )

						WIN.mpserver.playSong( this.getAttribute( 'data-song-path' ) )
		...
		MPTable.prototype.filter = function ( query, forceFresh )
		{
			...
			else
			{
				...
				// create regex for each query and push to array
				for ( var qi = 0, ql = queries.length; qi < ql; ++qi )
				{
					regexes.push( new RegExp( '\\b' + queries[ qi ], 'g' ) )
				}

				// match against anything
				loop1:
				for ( var i = 0, l = this.data.length, c, i2, i3, m; i < l; ++i )
				{
					c = this.data[ i ]
					...
					{
						// make sure the property exists
						if ( typeof c.data[ testAgainst[ i2 ] ] === 'undefined' )
						{
							continue loop2
						}
						...
						{
							// case insensitive test
							if ( regexes[ i3 ].test( c.data[ testAgainst[ i2 ] ].toString().toLowerCase() )
								// make sure this query hasn't already been matched
								&& m.indexOf( queries[ i3 ] ) === -1
								// make sure this isn't already in the results array
								&& filtered.indexOf( c ) === -1
							)
							{
								...
								// push to filtered if all queries are satisfied
								if ( m.length === ql )
								{
									filtered.push( c )

									// should be able to break loop2 here...
		...
		MPTable.prototype.render = function ( start, end )
		{
			...
			for ( var elem, cl; start <= end; ++start )
			{
				elem = this.filtered[ start ].elem

				cl = elem.classList

				// test for odd, which actually means even in this case because of the spacer row
				if ( start % 2 === 1 )
				{
					cl.add( 'mp-dt-row-even' )
				}

				else

				if ( cl.contains( 'mp-dt-row-even' ) )
				{
					cl.remove( 'mp-dt-row-even' )
				}

				this.tbody.insertBefore( elem, this.spacerBottom )
			}
		}
