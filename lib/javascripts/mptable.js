...
;( function ( WIN )
	{
		...
		/* MPTable.prototype.updateByPath = function ( path, data )
		{
			var that = this
		...
		MPTable.prototype.processQueue = function ()
		{
			...
			if ( l > this.maxInQueue || cd - this.lastProcessQueue > 750 )
			{
		...
		// web worker callback
		SORTER.addEventListener( 'message', function ( e )
			{
		...
		MPTable.prototype.sort = function ()
		{
			...
			for ( var i = 0, l = this.data.length; i < l; ++i )
			{
				...
				// set data
				obj.data = this.data[ i ].data

				obj.elemId = parseInt( this.data[ i ].elem.getAttribute( 'data-table-row-id' ) )

				elems[ obj.elemId ] = this.data[ i ].elem
