...
;( function ()
	{
		...
		Server.prototype.playSong = function ( path )
		{
			...
			if ( MPLAYER_PROCESS )
			{
				...
				MPLAYER_PROCESS.kill( 'SIGTERM' )
			}

			// create a new mplayer process
			MPLAYER_PROCESS = SPAWN( MPLAYER_PATH, [ '-slave', '-quiet', '-input', 'nodefault-bindings', '-noconfig', 'all', PATH.relative( __dirname, path ) ] )
		...
		function Server_scanFile( fileName, filePath, stats )
		{
			...
			if ( AUDIO_REGEX.test( mime ) )
			{
				...
				{
					if ( !evt.target.result || !evt.target.result.mtime || stats.mtime > evt.target.result.mtime )
					{
										...
										that.emit( 'scannedsong',
											{
												...
												duration: convertTime( Math.round( data.format.duration || 0 ) ),
