...
;( function ( WIN )
	{
		...
		{
			// test against indexedDB database

			//var songRow = WIN.mpserver.renderFile( 'song-row', data )
			...
			var request = objectStore.add( newSong )
			...
			//MPT.tables[ 'songs' ].addBulk( [ data ] )

			MPT.tables[ 'artists' ].addQueue( { artist: data.artist } )

			MPT.tables[ 'albums' ].addQueue( { album: data.album } ) */

			/* MPT.tables[ 'artists' ].add(
				{
					artist: data.artist
				}
			)

			MPT.tables[ 'albums' ].add(
				{
					album: data.album
				}
			)*/

			//TABLES[ 'songs' ].add( songRow )

			/* TBODIES.song.insertAdjacentHTML( 'beforeend', songRow )

			doAddArtistsAlbumsRow( 'artists', 'artist', data.artist )

			doAddArtistsAlbumsRow( 'albums', 'album', data.album ) */
		}
		...
		function doOnReady()
		{
			...
			AUDIO = DOC.getElementById( 'mp-audio' )
			...
			request.onupgradeneeded = function ( e )
			{
				...
				{
					var songsObjectStore = db.createObjectStore( 'mpsongs', { keyPath: 'id' } )

					// create some indices
					songsObjectStore.createIndex( 'path', 'path', { unique: true } )
