...
;( function ( WIN )
	{
		...
		// gets "stuck" on linux - works okay on windows
		// mouseup doesn't release the slider, also interferes with window drag sometimes
		var seekTimeouts = {}
		...
		function doSeekbarChange()
		{
			...
			// should set a quick timeout that clears itself on change
			// to minimize the amount of calls to seekToPercent

			if ( seekTimeouts.hasOwnProperty( this ) )
			{
				clearTimeout( seekTimeouts[ this ] )
			}

			var that = this

			seekTimeouts[ this ] = setTimeout( function ()
				{
					WIN.mpserver.seekToPercent( that.value )

					delete seekTimeouts[ that ]
				},

				// need to find a good value
				50
			)
