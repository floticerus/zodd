...
;( function ( WIN )
	{
		...
		function doServerSongFinished()
		{
			...
			// look for the next song in the filtered array
			var i = MPT.tables.songs.filtered.indexOf( WIN.mpserver.nowPlaying )
			...
			// look for song that's already playing
			var alreadyPlaying = MPT.tables.songs.tbody.getElementsByClassName( 'mp-song-playing' )

			// loop through them all to be sure there are no stragglers
			for ( var i2 = 0, l2 = alreadyPlaying.length; i2 < l2; ++i2 )
			{
				// remove playing class from row
				alreadyPlaying[ i2 ].classList.remove( 'mp-song-playing' )
			}

			// if next song isn't set yet, look for the first filtered result
			if ( !nextSong && MPT.tables.songs.filtered[ 0 ] )
			{
		...
		function doServerPlaying( data )
		{
			...
			for ( var i = 0, l = MPT.tables.songs.filtered.length; i < l; ++i )
			{
				...
				{
					WIN.mpserver.nowPlaying = MPT.tables.songs.filtered[ i ]

					// break whether or not next song has been defined
					break
				}
			}
		}
		...
		function doMplayerEvent( data )
		{
			switch ( data.key )
			{
				...
				:
					if ( SEEKBAR.classList.contains( 'mp-slider-active' ) )
					{
						break
					}
